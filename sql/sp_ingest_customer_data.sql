-- Automated customer data ingestion pipeline with validation, dedup, CDC, load tracking, error logging (DEPOC-32)
...
BEGIN
    -- STEP 1: Load files from stage into RAW_CUSTOMER_STAGING (AC-5 idempotent)
    ...
    -- Snapshot stream into temp table (stream consumed once per txn)
    ...
    SELECT COUNT(*) INTO :v_rejected_count FROM DE_POC.PUBLIC.CUSTOMER_ERROR_LOG WHERE LOAD_ID = :v_load_id;
    v_valid_count := v_staged_count - v_rejected_count;

    -- STEP 3: Deduplicate + MERGE (AC-1 incremental, AC-2 dedup)
    MERGE INTO DE_POC.PUBLIC.CUSTOMER AS tgt
    USING (
        SELECT * FROM (
            SELECT CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE, ADDRESS, CITY, STATE, ZIP_CODE, COUNTRY,
                TRY_TO_TIMESTAMP_NTZ(CREATED_DATE) AS CREATED_DATE, TRY_TO_TIMESTAMP_NTZ(UPDATED_DATE) AS UPDATED_DATE,
                _SOURCE_FILE, ROW_NUMBER() OVER (PARTITION BY CUSTOMER_ID ORDER BY UPDATED_DATE DESC, _LOAD_TIMESTAMP DESC) AS rn
            FROM DE_POC.PUBLIC.TEMP_CUSTOMER_BATCH
    ...
    WHEN MATCHED AND (
        tgt.FIRST_NAME <> src.FIRST_NAME OR tgt.LAST_NAME <> src.LAST_NAME
    ...
    SELECT COUNT(*) INTO :v_inserted_count FROM DE_POC.PUBLIC.CUSTOMER WHERE _LOAD_ID = :v_load_id AND _LOADED_AT = _UPDATED_AT;
    SELECT COUNT(*) INTO :v_updated_count FROM DE_POC.PUBLIC.CUSTOMER WHERE _LOAD_ID = :v_load_id AND _LOADED_AT <> _UPDATED_AT;
    v_dup_count := v_valid_count - v_inserted_count - v_updated_count;
    IF (v_dup_count < 0) THEN v_dup_count := 0; END IF;

    -- STEP 4: Update load tracking (AC-4)
    ...
    SET RECORDS_VALID = :v_valid_count, RECORDS_REJECTED = :v_rejected_count,
        ...
        RECORDS_DUPLICATE = :v_dup_count, PROCESSING_STATUS = 'SUCCESS', COMPLETED_AT = CURRENT_TIMESTAMP()
    WHERE LOAD_ID = :v_load_id;

    DROP TABLE IF EXISTS DE_POC.PUBLIC.TEMP_CUSTOMER_BATCH;

    RETURN OBJECT_CONSTRUCT('load_id', :v_load_id, 'source_file', :v_source_file, 'status', 'SUCCESS',
