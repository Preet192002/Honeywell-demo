-- Stored procedure for incremental customer data ingestion with validation and error handling
-- Co-authored with CoCo

USE DATABASE DE_POC;
USE SCHEMA PUBLIC;

CREATE OR REPLACE PROCEDURE SP_INGEST_CUSTOMER_DATA()
RETURNS VARIANT
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    -- Initialize load tracking record (AC-4)
    ...
    -- Ingest new files from stage into raw staging table
    ...
    -- Capture incremental delta via stream (AC-1: CDC processing)
    ...
    -- No new data: clean up tracking record and exit (AC-6: idempotent)
    ...
    -- Validation: reject invalid records and log errors (AC-3, AC-5)
    ...
    -- MERGE: deduplicate and upsert valid records (AC-1, AC-2, AC-7)
    ...
    -- Count inserts vs updates for load tracking
    ...
    -- Finalize load tracking record (AC-4)
    UPDATE DE_POC.PUBLIC.CUSTOMER_LOAD_TRACKING
...
EXCEPTION
    WHEN OTHER THEN
        -- Log error in tracking table (AC-5)
        UPDATE DE_POC.PUBLIC.CUSTOMER_LOAD_TRACKING
